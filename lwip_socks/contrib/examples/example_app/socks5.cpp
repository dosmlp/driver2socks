...
namespace tun2socks {
	...
	bool Socket5Client::connect(const std::string& proxy_ip, uint16_t proxy_port) {
		if (!_connected) {
			asio::ip::tcp::resolver::query q(proxy_ip.c_str(), std::to_string(proxy_port).c_str());
			...
			try {
				...
				_socket.send(asio::buffer(hello_msg, 3));
				u_char recv_msg[2];
				_socket.receive(asio::buffer(recv_msg, 2));
				...
				while (act != SUCCESS && act != FAILURE) {
					if (act == SEND) {
						...
						_socket.send(asio::buffer(p.get(), len));
						_auth->sent();
					}
					else if (act == RECV) {
						size_t len;
						auto p = _auth->construct_receive(len);
						_socket.receive(asio::buffer(p.get(), len));
	...
	bool Socket5Client::establish(const std::string& domain, uint16_t port) {
		...
		size_t recved;
		try {
			_socket.send(asio::buffer(request.data(), request.len()));
			recved = _socket.receive(asio::buffer(buffer, 2000));
		}
		catch (std::exception& e) {
	...
	void Socket5Client::sendAsync(std::shared_ptr<u_char> buffer, size_t len, std::function<SendAsyncCallback> handler) {
		_socket.async_send(asio::buffer(buffer.get(), len), _strand.wrap(handler));
	}

	void Socket5Client::receiveAsync(std::shared_ptr<u_char> buffer, size_t len, std::function<ReceiveAsyncCallback> handler) {
		_socket.async_receive(asio::buffer(buffer.get(), len), _strand.wrap(handler));
	}
