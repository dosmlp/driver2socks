...
namespace avpncore {
	...
	class tcp_stream
	{
			...
			int seq, int ack, uint8_t flags)
		{
			...
			*(uint16_t*)(tcp + 14) = 0xffff;				// window size
	...
	public:
		...
		tcp_stream(asio::io_context& io_context)
		...
		void output(const uint8_t* buf, int len)
		{
			...
			if (flags.flag.rst)
			{
				...
				{
					m_accept_handler(asio::error::network_reset);
			...
			if (payload_len > 0 && !keep_alive)
			{
				...
				auto target = asio::buffer_cast<void*>(
	...
	public:
		...
		asio::streambuf m_tcp_recv_buffer;
		bool m_accepted;
		bool m_do_closed;
		tsm m_tsm;
		bool m_abort;
