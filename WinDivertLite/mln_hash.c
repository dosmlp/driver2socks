...
int mln_hash_init(mln_hash_t *h, struct mln_hash_attr *attr)
{
    ...
    if (h->pool != NULL) {
        ...
        h->tbl = (mln_hash_mgr_t *)ExAllocatePoolQuotaZero(POOL_FLAG_NON_PAGED,h->len * sizeof(mln_hash_mgr_t),"mlnhash");
    }
...
mln_hash_new(struct mln_hash_attr *attr)
{
    ...
    if (attr->pool != NULL) {
        ...
        h = (mln_hash_t *)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(mln_hash_t), "mlnhash");
    }
...
mln_hash_entry_new(mln_hash_t *h, void *key, void *val)
{
    ...
    if (h->pool != NULL) {
        ...
        he = (mln_hash_entry_t *)ExAllocatePool2(POOL_FLAG_NON_PAGED,sizeof(mln_hash_entry_t),"mlnhash");
    }
    if (he == NULL) return NULL;
    he->val = val;
    he->key = key;
