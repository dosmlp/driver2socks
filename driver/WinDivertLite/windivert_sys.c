...
struct context_s
{
    ...
    mln_rbtree_t* rbtree_blockip;//需要拦截的本地IP和端口
    mln_rbtree_t* rbtree_appnames;//需要拦截的应用名称
};
...
static BOOL windivert_filter_network(PNET_BUFFER buffer, const VOID* layer_data, context_t context,
    BOOL ipv4, BOOL frag_mode, const WINDIVERT_FILTER* filter);
...
static void WinDivertHelperNtohIPv6Address(const UINT* inAddr, UINT* outAddr);
static void WinDivertHelperHtonIPv6Address(const UINT* inAddr, UINT* outAddr);
/*
  * Byte ordering.
  */
  /*
...
static int cmp_handler(const void* data1, const void* data2)
{
    ...
    WINDIVERT_DATA_SOCKET* s2 = (PWINDIVERT_DATA_SOCKET)data2;
    if (s1->Protocol > s2->Protocol) {
        return 1;
    } else if (s1->Protocol < s2->Protocol) {
        return -1;
    }
    //LocalPort
    if (s1->LocalPort > s2->LocalPort) {
        return 1;
    } else if (s1->LocalPort < s2->LocalPort) {
        return -1;
    }
    //RemotePort
    //if (s1->RemotePort > s2->RemotePort) {
    //    return 1;
    //} else if (s1->RemotePort < s2->RemotePort) {
    //    return -1;
    //}
    //LocalAddr

    for (int i = 0; i < 4; ++i) {
        if (s1->LocalAddr[i] > s2->LocalAddr[i]) {
            return 1;
        } else if (s1->LocalAddr[i] < s2->LocalAddr[i]) {
            return -1;
        }
    }
    //RemoteAddr
    //for (int i = 0; i < 4; ++i) {
    //    if (s1->RemoteAddr[i] > s2->RemoteAddr[i]) {
    //        return 1;
    //    } else if (s1->RemoteAddr[i] < s2->RemoteAddr[i]) {
    //        return -1;
    //    }
    //}
    return 0;
...
static struct mln_rbtree_attr ip_rbtree_attr = {
    .pool = NULL,
    .pool_alloc = NULL,
    .pool_free = NULL,
    .cmp = cmp_handler,
...
extern VOID windivert_destroy(IN WDFOBJECT object)
{
    ...
    if (context->rbtree_blockip != NULL) {
    ...
    OUT FWPS_CLASSIFY_OUT0 *result)
{
    ...
    RtlZeroMemory(&network_data, sizeof(WINDIVERT_DATA_NETWORK));
    BOOL loopback;
    ...
    OUT FWPS_CLASSIFY_OUT0 *result)
{
    ...
    do
    {
        ...
        */
        BOOL match = windivert_filter_network(buffer_fst, (PVOID)network_data, context, ipv4, frag_mode, filter);
        if (match)
        {
            break;
        }
    ...
    UINT64 flow_context)
{
    ...
    if (!mln_rbtree_null(node, context->rbtree_blockip)) {
        ...
        DEBUG("rbtree free %u %u", flow->data.LocalAddr[0], flow->data.LocalPort);
    }
    ...
    OUT FWPS_CLASSIFY_OUT0 *result)
{
    ...
    DEBUG("windivert_resource_release_v4_classify %u %u", socket_data.LocalAddr[0], socket_data.LocalPort);
    ...
    OUT FWPS_CLASSIFY_OUT0 *result)
{
    ...
    if (meta_vals->currentMetadataValues & FWPS_METADATA_FIELD_PROCESS_PATH) {
        UINT32 path_len = meta_vals->processPath->size / 2;
        RtlZeroMemory(socket_data.ProcessName, sizeof(socket_data.ProcessName));
        WCHAR* ppath = (WCHAR*)meta_vals->processPath->data;
        for (INT32 i = path_len - 1; i >= 0; --i) {
            if (RtlEqualMemory(ppath+i,L"\\", 2)) {
                ++i;
                RtlCopyMemory(socket_data.ProcessName, 
                    ppath + i, 
                    min(sizeof(socket_data.ProcessName)-2, (path_len - i) * 2));
                break;
            }
        }
    }
    ...
    BOOL outbound, BOOL loopback, FWPS_CLASSIFY_OUT0 *result)
{
    ...
    if (event == WINDIVERT_EVENT_SOCKET_CLOSE) {
        ...
        if (!mln_rbtree_null(node, context->rbtree_blockip)) {
            //不能在调用api关闭时就删掉ip,否则在挥手阶段client端的挥手包无法被拦截
            //mln_rbtree_delete(context->rbtree_blockip, node);
            //mln_rbtree_node_free(context->rbtree_blockip, node);
    ...
    BOOL ipv4, BOOL frag_mode, const WINDIVERT_FILTER* filter)
{
    ...
    network_data = (PWINDIVERT_DATA_NETWORK)layer_data;
    if (ipv4) {
        network_data->LocalAddr[0] = ip_header->SrcAddr;
        network_data->RemoteAddr[0] = ip_header->DstAddr;
    } else {
        network_data->LocalAddr[0] = ipv6_header->SrcAddr[0];
        network_data->LocalAddr[1] = ipv6_header->SrcAddr[1];
        network_data->LocalAddr[2] = ipv6_header->SrcAddr[2];
        network_data->LocalAddr[3] = ipv6_header->SrcAddr[3];
        network_data->RemoteAddr[0] = ipv6_header->DstAddr[0];
        network_data->RemoteAddr[1] = ipv6_header->DstAddr[1];
        network_data->RemoteAddr[2] = ipv6_header->DstAddr[2];
        network_data->RemoteAddr[3] = ipv6_header->DstAddr[3];
    }
    if (protocol == WINDIVERT_IP_PROTOCOL_TCP) {
        network_data->LocalPort = tcp_header->SrcPort;
        network_data->RemotePort = tcp_header->DstPort;
    } else if (protocol == WINDIVERT_IP_PROTOCOL_UDP) {
    ...
    RtlZeroMemory(&s, sizeof(WINDIVERT_DATA_SOCKET));
    if (ipv4) {
        s.LocalAddr[0] = WinDivertHelperNtohl(network_data->LocalAddr[0]);
        s.LocalAddr[1] = 0x0000FFFF;
    } else {
        WinDivertHelperNtohIPv6Address(network_data->LocalAddr, s.LocalAddr);
    }
    s.LocalPort = WinDivertHelperNtohs(network_data->LocalPort);
    s.Protocol = network_data->Protocol;
    mln_rbtree_node_t* node = mln_rbtree_search(context->rbtree_blockip, &s);
    if (!mln_rbtree_null(node, context->rbtree_blockip)) {
        return TRUE;
    }
    return FALSE;
