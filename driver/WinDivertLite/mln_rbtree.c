...
mln_rbtree_new(struct mln_rbtree_attr *attr)
{
    ...
    if (attr == NULL || attr->pool == NULL) {
        t = (mln_rbtree_t*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(mln_rbtree_t), "rbtree");
    } else {
...
mln_rbtree_node_new(mln_rbtree_t *t, void *data)
{
    ...
    if (t->pool == NULL)
        n = (mln_rbtree_node_t*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(mln_rbtree_node_t), "rbtree");
    else
